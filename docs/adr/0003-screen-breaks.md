...
### A dragged period obstructs nothing — 2026-09-05

The drag used to be an obstacle, and the consequence was written down here as deliberate: *while a device
stays unlocked and no rest happens, the owed chain parks at the now-line and the fill schedules no task under
it*. On account 3 that came out as a grey **"Inactivity" band growing behind the now-line** while the machine
was unlocked and every task was free to run — which `docs/scheduler_requirements.md` § *No idling* forbids
outright, and which the mode-1 clause forbids by name: the line reaching a pose *"would continuously delay
that period (while creating task panels in its passing)"*.

The accepted consequence was not a reading of the requirements, it was a reading of the materialization. A
dragged pose is ahead of the line at **every** position of the line, so no instant of the timeline is ever
inside it — it is a statement that a break is OWED, not a stretch that is blocked. Planning around it as
though it were a fixed block meant:

- the half-open `(t_p, t_p + d]` left exactly `[t_p, t_p + 1)` free, so the fill placed **one millisecond** of
  work at the line and then idled for the pose's whole length; and
- every later re-plan regenerated the pose at the NEW line, so the whole stretch the line had swept since the
  pose fell due came out of the fill with no panel at all. The calendar draws whatever the past leaves
  uncovered as a derived grey band, so the anomaly grew at exactly the rate the line moved.

So the fill now **draws** the dragged instance and does not plan around it (`isDraggedScreenBreak`, marked on
the panel id because a dynamic period's panel is derived and cut-and-regenerated by every fill; only
`restrictions` loses it). The break is still announced on its due, still drawn at `(t_p, t_p + d]`, and still
re-anchors the recurrence bars off itself — the drag goes on saying "you owe a break". What changed is that
the plan underneath it is the plan the passing line will actually realize, which is what makes the swept
stretch come out as task panels.

**Modes 2 and 3 keep the drag as an obstacle**, each for its own reason. Mode 3 does not drag at all: the
account said the break is being taken, so the pose elapses under the line and really happens. Mode 2 drags
exactly as mode 1 does, but its rule is that `t_p` IS covered by "no on-screen task" — the passing there
creates COVERAGE, not task panels — so the grey band behind the line is what "no device of this account is
unlocked" is supposed to look like, and an on-screen task must not be planned into it. That is the one place
the two modes' answers to the drag differ, and `DraggedPoseNoIdlingTest` pins all three.

The owed chain still clears the way it always did: the app conducts a break and records it (a pre-placed
period, so never dragged), or the user actually goes away and the mode flips.
